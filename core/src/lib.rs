...
pub enum RunState { Idle, Running, Paused }
...
impl PomodoroPlan {
    pub fn default_plan() -> Self {
        Self { work_min: 25, short_break_min: 5, long_break_min: 15, cycle_len: 4, auto_start: true }
    }
}
...
impl Session {
    fn elapsed_ms(&self) -> u64 {
        match self.run_state {
            RunState::Running => {
                // Monotonic time tracking with drift correction
                // If system sleeps, Instant continues from sleep point,
                // so this naturally handles sleep/wake scenarios
    ...
    fn total_ms(&self) -> u64 { phase_ms(&self.plan, self.phase) }
    ...
    /// Auto-advance to next phase if current phase is complete and auto_start is enabled
...
fn next_phase(plan: &PomodoroPlan, last_cycle_index: u8) -> (Phase, u8) {
    if last_cycle_index >= plan.cycle_len { (Phase::LongBreak, plan.cycle_len) } else { (Phase::ShortBreak, last_cycle_index) }
}

fn after_break(plan: &PomodoroPlan, last_cycle_index: u8) -> (Phase, u8) {
    let next = if last_cycle_index >= plan.cycle_len { 1 } else { last_cycle_index + 1 };
    (Phase::Work, next)
}
...
pub fn start_work(plan: PomodoroPlan) -> SessionSnapshot {
    println!("Rust: start_work called with plan: {:?}", plan);
    let mut guard = ENGINE.lock();
    let (cycle_index, plan_to_use) = match &*guard {
        Some(s) => (if matches!(s.phase, Phase::Work) { s.cycle_index } else { s.cycle_index }, s.plan.clone()),
    ...
    *guard = Some(s);
    println!("Rust: Session created, returning snapshot");
    snapshot()
}

#[frb]
pub fn pause() -> SessionSnapshot {
...
#[frb]
pub fn extend_work(extra_minutes: u16) -> SessionSnapshot {
    let mut g = ENGINE.lock();
    if let Some(s) = g.as_mut() {
        if let Phase::Work = s.phase {
            s.plan.work_min = s.plan.work_min.saturating_add(extra_minutes);
        }
    }
    snapshot()
}
...
#[frb]
pub fn skip() -> SessionSnapshot {
    let mut g = ENGINE.lock();
    if let Some(s) = g.as_mut() {
        match s.phase {
            Phase::Work => {
                let (p, _) = next_phase(&s.plan, s.cycle_index);
                s.phase = p;
                s.run_state = if s.plan.auto_start { RunState::Running } else { RunState::Paused };
                s.started_at = Instant::now();
                s.carried_ms = 0;
            }
            Phase::ShortBreak | Phase::LongBreak => {
                let (p, next_idx) = after_break(&s.plan, s.cycle_index);
                s.phase = p;
                s.cycle_index = next_idx;
                s.run_state = if s.plan.auto_start { RunState::Running } else { RunState::Paused };
                s.started_at = Instant::now();
                s.carried_ms = 0;
            }
...
pub fn snapshot() -> SessionSnapshot {
    ...
    if let Some(s) = g.as_mut() {
        ...
        println!("Rust: snapshot with session - {:?}", result.run_state);
        result
    } else {
        println!("Rust: snapshot with no session - idle");
        SessionSnapshot {
            session_id: String::new(),
            phase: Phase::Work,
            run_state: RunState::Idle,
